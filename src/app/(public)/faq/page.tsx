      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <AnimateIn>
          <div className="text-center">
            ...
            <h1 className="mt-2 font-serif text-4xl font-bold text-plum-dark sm:text-5xl">Frequently Asked Questions</h1>
            <p className="mt-4 text-muted-foreground">Everything you need to know about our services and booking process.</p>
          </div>
        </AnimateIn>
          ...
          <Accordion type="single" collapsible className="mt-14">
            {FAQ_ITEMS.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                <AccordionTrigger className="text-left font-medium text-plum-dark transition-colors hover:text-primary">{item.question}</AccordionTrigger>
        ...
        <AnimateIn delay={300}>
          <div className="mt-14 rounded-2xl bg-secondary/50 p-8 text-center">
            <h2 className="font-serif text-xl font-semibold text-plum-dark">Still have questions?</h2>
            <p className="mt-2 text-sm text-muted-foreground">We&apos;re happy to help! Reach out to us directly.</p>
            <Button asChild className="mt-5 rounded-full px-8">
              <Link href={`mailto:${BUSINESS_INFO.email}`}><Mail className="mr-2 h-4 w-4" />Contact Us</Link>
            </Button>
          </div>
        </AnimateIn>
      </div>
